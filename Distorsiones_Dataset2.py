...
import os
list_fichero = os.listdir('DibujosMuestra/dataset2_reyCopia/reyCopia')
logger = logging.getLogger(__name__)

MIN_MATCH_COUNT = 1

# Vamos a leer y generar detectores para el modelo en primer lugar.
img1 = cv2.imread('DibujosMuestra/dataset2_reyCopia/rey_modelo.png', flags=cv2.IMREAD_GRAYSCALE)
for i, fichero in enumerate(list_fichero):
    i = str(i) + '_dts2'
    img2 = cv2.imread('DibujosMuestra/dataset2_reyCopia/reyCopia/'+fichero, flags=cv2.IMREAD_GRAYSCALE)
    t0_img1 = time.time()
    lit_metodos = []
